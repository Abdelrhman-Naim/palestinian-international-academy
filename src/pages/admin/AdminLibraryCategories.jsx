...
export default function AdminLibraryCategories() {
  ...
  const handleSaveEdit = async (e) => {
    ...
    const res = await updateCategory('library', editModal.item, trimmed);
    if (!res.ok) {
  ...
  return (
    <AdminPageShell
      parent={t('adminLibraryCategories.parent')}
      title={t('adminLibraryCategories.title')}
      subtitle={t('adminLibraryCategories.subtitle')}
      icon="category"
    >
      <form
        onSubmit={handleAdd}
      ...
      >
        <input
          ...
          placeholder={t('adminLibraryCategories.placeholder')}
          className="flex-1 rounded-xl border border-[#E8E2D5] bg-white px-4 py-3 text-sm text-dark outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/20 dark:border-gray-700 dark:bg-gray-900 dark:text-white"
        />
        <button
          type="submit"
      ...
      {error && (
        <div className="mb-4 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-bold text-rose-600 dark:border-rose-800 dark:bg-rose-900/20">
          {error}
        </div>
      )}
            ...
            >
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="rounded-lg bg-primary/10 dark:bg-primary/15 text-primary dark:text-primary px-3 py-2 text-sm font-bold transition-colors hover:bg-primary/20 flex items-center gap-1.5"
                >
                  <span className="material-symbols-outlined text-[16px]">edit</span>
                  <span>{t('adminLibraryCategories.edit') || t('common.edit')}</span>
                </button>
                <button
                  type="button"
                  onClick={() => setPendingDelete(item)}
                  className="rounded-lg bg-rose-50 px-3 py-2 text-sm font-bold text-rose-600 transition-colors hover:bg-rose-100 dark:bg-rose-950/40 dark:text-rose-400 dark:hover:bg-rose-900/40 flex items-center gap-1.5"
                >
      ...
      {editModal && (
        ...
        >
          ...
          >
            <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-primary/10 text-primary dark:bg-primary/20 border border-primary/20">
            ...
            <p className="mt-1.5 text-center text-xs text-gray-500 dark:text-gray-400">
              {t('adminLibraryCategories.editSubtitle')}
            </p>
                ...
                >
                  {editModal.saving ? (
                    <>
                      ...
                      <span>{t('adminLibraryCategories.saving')}</span>
                    </>
                  ) : (
                    <span>{t('adminLibraryCategories.saveEdit')}</span>
                  )}
                </button>
              </div>
            </form>
      ...
      {pendingDelete && (
        ...
        >
          ...
          >
            ...
            <h3 className="text-center text-xl font-bold text-dark dark:text-white">{t('adminLibraryCategories.deleteTitle')}</h3>
            <p className="mt-2 text-center text-sm text-gray-500 dark:text-gray-400">
              {t('adminLibraryCategories.deleteConfirm')}
            </p>
            <div className="mt-7 flex gap-3">
              ...
              <button
                type="button"
                onClick={() => {
                  removeCategory('library', pendingDelete);
